...
import { stringifyContent, extractMessages, toMarkdown } from '../src/export.js';

test('stringifyContent handles arrays and object fallbacks', () => {
  assert.equal(stringifyContent(['hello', { text: 'world' }, { value: '!' }]), 'hello\nworld\n!');
  assert.equal(stringifyContent({ text: 'assistant reply' }), 'assistant reply');
...
test('toMarkdown renders chat metadata and role sections', () => {
  const md = toMarkdown({
    ...
    detail: { tree: [{ role: 'user', text: 'Question?' }, { role: 'assistant', content: 'Answer.' }] }
  });
